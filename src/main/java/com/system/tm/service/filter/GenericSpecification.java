package com.system.tm.service.filter;
...
import static com.system.tm.service.filter.Operation.IS_NOT_NULL;
import static com.system.tm.service.filter.Operation.IS_NULL;
...
public class GenericSpecification<T> implements Specification<T> {
    ...
    private LocalDateTime toDate(final Object value) {
        if (value == null) {
            throw new RuntimeException("Неподдерживаемое значение даты: null");
        }

        if (value instanceof LocalDateTime localDateTime) {
            return localDateTime;
        } else if (value instanceof LocalDate localDate) {
            return LocalDateTime.of(localDate, LocalTime.now()); // or a specific default time.
        } else if (value instanceof Date date) {
            return toLocalDateTime(date);
        } else if (value instanceof String s) {
            return parseStringToDate(s);
        } else {
            throw new RuntimeException("Неподдерживаемое значение даты: " + value.getClass());
        }
    }

    private LocalTime toTime(final Object value) {
        if (value == null) {
            throw new RuntimeException("Неподдерживаемое значение времени: null");
        }

        if (value instanceof LocalTime localTime) {
            return localTime;
        } else if (value instanceof String s) {
            return parseStringToTime(s);
        } else {
            throw new RuntimeException("Неподдерживаемое значение времени: " + value.getClass());
        }
    }
